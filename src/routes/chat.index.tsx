    ...
    <>
      ...
      <div className="space-y-2 px-4 pt-2 pb-32">
        ...
        {user && !isLoading && data.length === 0 && (
          <EmptyState icon={MessageCircle} title="Пока нет диалогов" description="Откройте объявление и нажмите «Написать», чтобы начать переписку с владельцем." />
        )}
        {data.map((c) => (
          <Link key={c.id} to="/chat/$id" params={{ id: c.id }} className="flex items-center gap-3 rounded-2xl bg-card p-3 ring-1 ring-border">
            {c.properties?.photos[0] ? <img src={c.properties.photos[0]} alt="" className="h-12 w-12 rounded-xl object-cover"/> : <div className="h-12 w-12 rounded-xl bg-muted"/>}
            <div className="min-w-0 flex-1">
              <div className="truncate font-display font-semibold">{c.properties?.title ?? "Диалог"}</div>
